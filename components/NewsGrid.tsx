          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {articles.slice(0, 3).map((item) => {
              ...
              return (
                ...
                >
                  {/* Thumbnail */}
                  <Link href={`/warta/${item.slug || item.id}`} prefetch={false} className="block overflow-hidden bg-gray-50 aspect-video relative">
                    {imgUrl ? (
                      <Image
                        src={imgUrl}
                        alt={item.title}
                  ...
                  <div className="bg-white p-5 rounded-b-2xl flex flex-col justify-between flex-1">
                    <div>
                      ...
                      <h3 className="text-base font-serif font-bold text-[#272522] line-clamp-2 mb-2 group-hover:text-[#E52E2D] transition-colors leading-snug">
                        <Link href={`/warta/${item.slug || item.id}`} prefetch={false}>
                          {item.title}
                        </Link>
                      </h3>
